...
limitawal = 100
memberlimit = 5
cr = '*KAMU SUDAH LOGIN DI DATABASE*'
/*************************************/
mess = {
				wait: '*「 WAIT 」 Lagi Dalam Proses Pembuatan*'
}
/******** OWNER NUMBER**********/
	...
	client.on('message-new', async (mek) => {
       ...
       case 'play':
               ...
               if (data.error) return reply(data.error)
                 infomp3 = `「 *PLAY* 」\n*Judul* : ${data.result.title}\n*Duration* : ${data.result.duration}\n*Filesize* : ${data.result.size}\n\n*Tunggu MIMIN Sedang mengirim audio nya jangan spam*`
               ...
               case 'kicktime':
				...
				case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ 𝗠𝗲𝗻𝗴𝗮𝗸𝘁𝗶𝗳𝗸𝗮𝗻 𝗳𝗶𝘁𝘂𝗿 𝗻𝘀𝗳𝘄 𝗱𝗶 ??𝗿??𝘂𝗽 𝗶𝗻𝗶')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
