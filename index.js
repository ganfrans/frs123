...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hallo @${num.split('@')[0]}\Selamat datang di group *${mdata.subject}* Silahkan intro terlebih dahulu ya
Nama:
Umur:
Asal kota:
Jenis kelamin:
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           await limitAdd(sender)                           
                   break
					...
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (args.length < 1) return reply('Hayoo. Mau apa senpai 😐')
					if (Number(args[0]) === 1) {
						if (isEventon) return reply('*SUDAH AKTIF* !!!')
						event.push(from)
						fs.writeFileSync('./database/bot/event.json', JSON.stringify(event))
						reply('*❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ 𝗠𝗲𝗻𝗴𝗮𝗸𝘁𝗶𝗳??𝗮𝗻 EVENT 𝗱𝗶 𝗴𝗿𝗼𝘂𝗽 𝗶𝗻𝗶️*')
				...
				case 'opromote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`𝗦𝗲𝗹𝗮𝗺𝗮𝘁 @${mentioned[0].split('@')[0]} *SEKARANG SENPAI ADALAH ADMIN GROUP* (+_+)`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
					     case 'randomhentai':
                                       ...
                                       reply(ind.wait)
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: '*RANDOM HENTAI*'})
                                        await limitAdd(sender)
                                        break
          case 'randomnekonime':
