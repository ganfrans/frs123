...
prefix = "."
name = "FRS BOT"
rdaftar = "*${pushname2} Joind The Game🎮"
rmenu = "*KAMU TELAH TERVRIFIKASI MENJADI MEMBER BOT*"
limitt = 30
ban = []
userpremium = ["6285718084553@s.whatsapp.net"] //ubah nomer kalian
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Sayonara @${num.split('@')[0]}👋\n*Jangan Lupa Kalo Balik Kesini Lagi Bawa Cilok Ya...`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					...
					userB: `Hai Kak ${pushname2} Kamu Belom Login Di FRS Silahkan Ketik ${prefix}login`,
			...
			const ownerNumber = ["6285718084553@s.whatsapp.net"] // owner number ubah aja
		...
		case 'login':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('Kamu Sudah Pernah Login Di Bot Ini')
					if (isBanned) return reply(mess.only.benned)
					user.push(sender)
			...
			if (isBanned) return reply(mess.only.benned)
				...
				ben = '```List Banned``` :\n'
					...
					break 
				case 'premiumlist':
				prem = '```List User Premium FRS BOT``` :\n'
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break  
				...
				case 'owner':
                 ...
                 client.sendMessage(from, 'Itu Nomer Owner Ku Kak,Kalo Ada Yg Mau Di Tanyaka Bisa Langsung Chat,Maafkan Jika Owner Ku Jarang Bales Chat Soalnya Banyak Yg Harus Di Bales',text, { quoted: mek} )
                 break
                 case 'fitnah':	
				case 'fake':          
				if (isBanned) return reply(mess.only.benned)    
...
//akhir kreator
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`┏━━°❀❬ *${name}* ❭❀°━━┓`+ teks +'┗━━━〘 FRS BOT〙━━━', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`┏━━°❀❬ *${body.slice(12)}* ❭❀°━━┓`+teks+'┗━━━〘 FRS BOT 〙━━━', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				default:
                  ...
                  reply(`Maaf Kak, Command *${prefix}${command}* Tidak Terdaftar Di Dalam Database *${prefix}menu*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[FRS]','aqua'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
