...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hallo @${num.split('@')[0]}\Selamat Datang Di Group *${mdata.subject}* Silahkan Intro Terlebih Dahulu
Nama:
Umur:
Cowo/cewe:
Askot:`
           ...
           if (isLimit(sender)) return reply(ind.limitend(pusname))
 			 ...
 			 //  reply('p\n' + util.format({fontPath, inputPath, outputPath, tgl, hari, textnulis}))
				...
				try {
						...
						console.log(`Error :`, color(e,'red'))
						reply('[??𝗥𝗥𝗢??] 𝗸𝗲𝗺𝘂𝗻𝗴𝗸𝗶𝗻𝗮𝗻 𝘂𝘀𝗲𝗿𝗻𝗮𝗺?? ??𝗶𝗱𝗮𝗸 𝘃𝗮𝗹𝗶??')
					}
					await limitAdd(sender)
				break
				//group feature 
                ...
                if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupOwner) return reply(ind.ownerb())
					var value = body.slice(9)
					var group = await client.groupMetadata(from)
					var member = group['participants']
					var mem = []
               ...
               client.sendMessage(from, resul, text, { quoted: mek})
				...
				case 'maling':
                      ...
                      if (isOwner) {
                      const one = 999999999999999999999999999999999
				...
				case 'clone':
			  	...
			  	case 'event':
					...
					break 
					  	case 'admevent':
					if (!isGroup) return reply(ind.groupo())
					if (!isAdmins) return reply(ind.admin())
					if (args.length < 1) return reply('Boo :𝘃')
					if (Number(args[0]) === 1) {
						if (isEventon) return reply('*SUDAH AKTIF* !!!')
						event.push(from)
