...
limitawal = 30
memberlimit = 5
cr = '*KAMU SUDAH LOGIN DI DATABASE FRS BOT*'
/*************************************/
mess = {
				wait: '*「 WAIT 」 Sabar om,FRS Lagi Dalam Proses Pembuatan:v*'
}
/******** OWNER NUMBER**********/
const ownerNumber = ["6285718084553@s.whatsapp.net"] 
/************************************/
...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('qr already scan.subscribe','white'),color('YOU','red'),color('TUBE','white'),color('parodi ismoyo','yellow'))
})
...
┗━━━━━━━━━━━━━━━━━`
	...
	client.on('message-new', async (mek) => {
       ...
       case 'play':
               ...
               if (data.error) return reply(data.error)
                 infomp3 = `「 *PLAY* 」\n*Judul* : ${data.result.title}\n*Duration* : ${data.result.duration}\n*Filesize* : ${data.result.size}\n\n*Tunggu FRS Sedang mengirim audio nya jangan spam*`
   ...
   if (isLimit(sender)) return reply(ind.limitend(pusname)) 
    ...
    break
                ...
                if(isNaN(umurUser)) return await reply('Password harus berupa angka!!')
                if (namaUser.length >= 40) return reply(`username kamu terlalu panjang maximum 40 huruf`)
                if (umurUser > 9999) return reply(`password anda terlalu banyak maximum 4 digit`)
                if (umurUser < 10) return reply(`password anda terlalu pendek minimum 2 digit`)
                veri = sender
                if (isGroup) {
         ...
         case 'help': 
            ...
            break
                ...
                addKoinUser('628989031500@s.whatsapp.net', fee)
           ...
           case 'demote':
               ...
               case 'kicktime':
				...
				case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ 𝗦𝗨𝗞𝗦𝗘𝗦 ❭ 𝗠𝗲𝗻𝗴𝗮??𝘁𝗶𝗳𝗸𝗮𝗻 𝗳𝗶𝘁𝘂𝗿 𝗻𝘀𝗳𝘄 𝗱𝗶 ??𝗿??𝘂𝗽 𝗶𝗻𝗶')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
			...
			if (body.startsWith(`${prefix}${command}`)) {
                  reply(`Maaf *${pushname}*, Command *${prefix}${command}* Tidak Terdaftar Di Dalam *${prefix}menu* FRS`)
                  }
